...
const Dashboard = () => {
  ...
  const fetchDashboardData = async (providedToken = null) => {
    console.log('=== FETCH DASHBOARD DATA START ===');
    
    // Don't change loading state if we're already authenticated
    if (!isAuthenticated) {
      setIsLoading(true);
    }
    setError(null);
    ...
    try {
      ...
      // REMOVED PROBLEMATIC TOKEN EXPIRATION CHECK
      // This was likely causing premature logouts
      ...
      // Fetch top users and notifications - but don't let them block the main data
      fetchTopUsers(token);
  ...
  const fetchTopUsers = async (providedToken = null) => {
    try {
      const token = providedToken || localStorage.getItem('token');
      if (!token) return;
      ...
      if (response.ok) {
        ...
        // Only redirect if we're not already handling auth issues
        if (isAuthenticated) {
          console.log('Token expired during top users fetch');
          localStorage.removeItem('token');
          localStorage.removeItem('user');
          setIsAuthenticated(false);
          window.location.href = '/login';
  ...
  const fetchNotifications = async (providedToken = null) => {
    try {
      const token = providedToken || localStorage.getItem('token');
      if (!token) return;
      ...
      if (response.ok) {
        ...
        // Only redirect if we're not already handling auth issues
        if (isAuthenticated) {
          console.log('Token expired during notifications fetch');
          localStorage.removeItem('token');
          localStorage.removeItem('user');
          setIsAuthenticated(false);
          window.location.href = '/login';
  ...
  // Don't render anything until we've completed the auth check
  if (authCheckInProgress.current) {
    return (
      <div className="dashboard-container">
        <div className="loading-spinner">
